...
| **Greene King** managed (greeneking.co.uk) | 753 | **Adapter built** (`--site greeneking`). Sitemap lists every pub; the pub page carries schema.org address and coordinates; `/our-beers` is server-rendered and names the cask ales on at that pub (Greene King IPA, Abbot Ale and the current guest). Checked across counties: it is the same three at every managed pub, so treat it as the company's core cask range stated per pub, not a pub-by-pub list. The full drinks list on `/menu` comes from an API behind Akamai bot protection that refuses non-browser clients; not worked around. Hungry Horse, Chef & Brewer, Flaming Grill and Farmhouse Inns share the platform but have no `/our-beers` page. |
